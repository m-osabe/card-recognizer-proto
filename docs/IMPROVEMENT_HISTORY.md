...
timeline
    ...
    section Phase 4.1 (施策①完了・現在)
      施策 : card-6/7 色相タイブレーカー & 連動選出 & ソート適正化
      成果 : 正答率 88.4% → 89.0% (706/793, 目標90%まであと8枚)
    section Phase 4.2 (次回実装)
      目標 : 正答率 90% 〜 92% 突破
...
| **Phase 4.1**<br>*(現在)* | **同型カード競合の解消**<br>(card-6 vs card-7) | 外枠75点共通による逆転、インライア優先ソート、粗探索の足切り | **イラスト色相・明度タイブレーカー**、同型カード連動選出、`combined_score` 優先ソート | **0.50s 前後** | **88.4% $\rightarrow$ 89.0%**<br>(706/793, **あと8枚**) | [**Phase 4.1 詳細**](05_phase4_accuracy_improvement_90plus.md) ★ |
| **Phase 4.2**<br>*(次回予定)* | **正答率 90% 以上の達成**<br>(必要追加正解: +8枚) | 手持ち撮影時の角丸・指かぶりによる枠検出全滅 (card-6/10) | **枠検出器への ConvexHull 凸包導入** ＆ クロップ探索フォールバック | **0.50s 前後見込** | **90.5% 〜 92.5%**<br>(**目標90%突破**) | *(次回作成予定)* |
