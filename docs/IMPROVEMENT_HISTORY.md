...
timeline
    ...
    section Phase 4 (事前調査・現在)
      課題 : 残る失敗92件(11.6%)の要因解明と90%突破
      分析 : 入力起因(ブレ) 21件 vs ロジック改善可能 71件
      成果 : 改善前ベースライン確定 (あと13枚で90%達成)
    section Phase 4.1 (次回実装)
      目標 : 正答率 90% 〜 95% 突破
      施策 : card-6 vs card-7 色相タイブレーカー & 枠検出マルチ二値化
```
...
| **Phase 4.1**<br>*(次回予定)* | **正答率 90% 以上の達成**<br>(必要追加正解: +13枚) | card-6/7のイラスト色相未活用、手持ち写真での枠取り失敗 | **イラスト色相(Hue)タイブレーカー**、枠検出器のマルチ二値化 ＆ 中央クロップフォールバック | **0.50s 前後見込** | **90.7% 〜 93.0%**<br>(目標突破) | *(次回作成予定)* |
