...
- [**アルゴリズム設計・技術選定書 (docs/ALGORITHM_DESIGN.md)**](docs/ALGORITHM_DESIGN.md)
  ...
  - 将来的な拡張性（OCR追加・数万枚へのスケール・Webアプリ化）

---

## 🌟 特徴

- **GPU不要・完全CPU動作**：OpenCV SIFT特徴点照合 ＋ 局所テクスチャ・カラーベクトル類似度検索を組み合わせ、一般的なPCのCPUで1枚あたり数十ミリ秒〜数百ミリ秒で軽快に動作します。
- **事前学習（トレーニング）不要**：新しいカードを追加する際は、各カード**1枚の画像**（公式画像やスキャン画像）を `data/master/` に置くだけで即座に認識対象になります。
- **斜め撮影・回転・影への耐性**：
  1. `CardDetector` が写真からカードの四隅を自動検出し、透視変換（Perspective Warp）で正面画像に歪み補正。
  2. SIFT のスケール・回転不変特徴点と RANSAC Homography 検証により、多少の傾きや指の映り込みがあっても正確に同定。
- **比較レポートの自動生成**：「入力写真」「検出・補正カード」「特定されたマスターカード」を1枚にまとめた視覚レポート画像を `output/` に自動保存。
...
├── matcher_sift.py       # OpenCV SIFT による特徴点・幾何学的整合性（Homography）照合
├── matcher_embedding.py  # カラー・テクスチャ多次元ベクトルによる類似度検索
├── card_engine.py        # 検出・照合・アンサンブル・可視化を統合するメインエンジン
├── cli.py                # コマンドライン実行ツール
...
### 1. デモサンプルで即座に動作確認する
手元にカード画像がなくても、自動生成された10種のカードと斜め撮影写真で即座に実験できます。
...
### 2. 単一の写真を判定する
```bash
python cli.py identify data/test/test_photo_001_Flame_Dragon.jpg
```
...
1. **マスター画像の配置**:
   ...
   - ファイル名がカード名やIDとして認識されます（例: `001_BlueEyesWhiteDragon.jpg`, `Charizard.png`）。
...
- `--method ensemble` (デフォルト): SIFT幾何整合性 (70%) ＋ カラー・テクスチャ類似度 (30%) の統合判定。最も高精度。
